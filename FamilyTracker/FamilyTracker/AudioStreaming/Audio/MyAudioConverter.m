...
static int CheckError(OSStatus error, const char *operation) {
	if (error == noErr)
...
-(BOOL)convert{
    ...
    OSStatus status = ExtAudioFileOpenURL((__bridge CFURLRef)sourceURL, &settings.inputFile);
    BOOL isError = CheckError(status, "ExtAudioFileOpenURL failed");
    if (isError) {
        return NO;
    }
	
	isError = [self validateInput:&settings];
    if (isError) {
        return NO;
    }
...
-(int)validateInput:(ExtAudioConverterSettings*)settigs{
