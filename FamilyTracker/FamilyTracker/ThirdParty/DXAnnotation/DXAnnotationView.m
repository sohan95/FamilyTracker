...
- (void)positionSubviews {
    ...
    if (_hasCalloutView) {
        ...
        frame.origin.y = -frame.size.height - self.settings.calloutOffset+20.0;
        frame.origin.x = (self.frame.size.width - frame.size.width) / 2.0;
//        float xPos = (self.frame.size.width - frame.size.width) / 2.0;
//        float frameSizeWidth = self.frame.origin.x;
//        if(xPos < 100) {
        ...
        if (self.calloutView.isHidden) {
            ...
            case DXCalloutAnimationZoomIn: {
                ...
                [UIView animateWithDuration:self.settings.animationDuration animations:^{
                    ...
                    NSLog(@"AnnotationView=%f",self.frame.origin.x);
                    NSLog(@"Window=%f",self.window.frame.origin.x);
                    ///
                    CGRect frame = self.calloutView.frame;

                    float calloutXOffset1 = ((self.frame.size.width - frame.size.width) / 2.0)+ self.frame.origin.x;
                    float calloutXOffset2 = frame.size.width + self.frame.origin.x;
                    
                    if (calloutXOffset1<0) {
                        frame.origin.x = self.pinView.frame.size.width+10;
                    }
                    else if (calloutXOffset2 > self.window.frame.size.width) {
                        float dif = calloutXOffset2-self.window.frame.size.width;
                        frame.origin.x = self.pinView.frame.origin.x-dif;
                    }

                   
                    self.calloutView.frame = frame;
                     NSLog(@"%f",self.calloutView.frame.origin.x);
                    ///
//                    self.calloutView.frame = CGRectMake(self.frame.origin.x, self.calloutView.frame.origin.y, self.calloutView.frame.size.width, self.calloutView.frame.size.height);
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (hitView == self)
        return nil;
    //NSLog(NSStringFromCGPoint(hitView.frame.origin));
    return hitView;
}
